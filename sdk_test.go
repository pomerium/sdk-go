...
func TestVerifier_GetIdentity(t *testing.T) {
	...
	tests := []struct {
		...
		{"can't parse malformed JWT", "", new(10), nil, nil, nil, context.TODO(), "", defaultAttestationHeader, nil, nil, "malformed", false, `{"error":"couldn't parse raw JWT: square/go-jose: compact JWS format must have three parts"}`},
		{"bad signing key", ts.URL, new(10), nil, nil, nil, context.TODO(), "", defaultAttestationHeader, badSigner, &Identity{Email: "user@pomerium.com"}, "", false, `{"error":"invalid JWT signature: square/go-jose: error in cryptographic primitive"}`},
		...
		{"does not pass iss validation", ts.URL, new(10), nil, nil, &jwt.Expected{Issuer: "pomerium"}, context.TODO(), "", defaultAttestationHeader, goodSigner, &Identity{Email: "user@pomerium.com"}, "", false, "{\"error\":\"unexpected claim: square/go-jose/jwt: validation failed, invalid issuer claim (iss)\"}"},
